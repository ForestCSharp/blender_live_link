...
vec3 neighborhood_min(vec2 in_uv)
{
	...
	vec3 result = texture(current_color_tex, in_uv).rgb;
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2(-1.0, -1.0)).rgb);
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2( 0.0, -1.0)).rgb);
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2( 1.0, -1.0)).rgb);
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2(-1.0,  0.0)).rgb);
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2( 1.0,  0.0)).rgb);
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2(-1.0,  1.0)).rgb);
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2( 0.0,  1.0)).rgb);
	result = min(result, texture(current_color_tex, in_uv + texel_size * vec2( 1.0,  1.0)).rgb);
...
vec3 neighborhood_max(vec2 in_uv)
{
	vec2 texel_size = 1.0 / screen_size;
	vec3 result = texture(current_color_tex, in_uv).rgb;
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2(-1.0, -1.0)).rgb);
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2( 0.0, -1.0)).rgb);
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2( 1.0, -1.0)).rgb);
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2(-1.0,  0.0)).rgb);
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2( 1.0,  0.0)).rgb);
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2(-1.0,  1.0)).rgb);
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2( 0.0,  1.0)).rgb);
	result = max(result, texture(current_color_tex, in_uv + texel_size * vec2( 1.0,  1.0)).rgb);
...
vec3 sharpen_current(vec2 in_uv, vec3 in_current_color, vec3 in_min_color, vec3 in_max_color)
{
	...
	vec3 near_avg = 0.5 * (
		texture(current_color_tex, in_uv - axis).rgb +
		texture(current_color_tex, in_uv + axis).rgb
	);
	vec3 far_avg = 0.5 * (
		texture(current_color_tex, in_uv - axis * 2.0).rgb +
		texture(current_color_tex, in_uv + axis * 2.0).rgb
	);
...
void main()
{
	...
	if (accepted_history)
	{
		history_color = texture(history_tex, previous_uv).rgb;
