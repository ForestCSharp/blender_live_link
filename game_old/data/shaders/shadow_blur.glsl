...
layout(binding=0) uniform texture2DArray color_tex;
...
void main()
{
	...
	for (int i = 0; i < blur_size; ++i)
	{
		...
		result += texture(sampler2DArray(color_tex, tex_sampler), vec3(uv + offset, float(array_layer))) * weight;
		total_weight += weight;
	}
